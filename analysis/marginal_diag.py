"""
Convergence diagnostics (ESS and R-hat) of the marginal-comparison series, k5model_mixture.

Source: data/out/k5model_mixture/marginal_diagnostics.csv (src.summaries ->
...
The table mixes two row shapes; this module unifies them to one (ESS, Rhat) per row:
  ...
  * kind == 'density'  - the per-draw marginal density f(x) is a SERIES over draws evaluated
    ...
    density_agg='mean' switches to the average-over-points summary instead.
  * kind == 'moment_Mean' / 'moment_Var' - a single scalar series per draw (the mixture mean /
    variance), so ESS and Rhat are already single values.
...
Run from the repo root with the project venv:
    .venv/Scripts/python.exe analysis/marginal_diag.py
"""
...
DIR_OUT_PLOTS = "marginal_comparison/plots"
DIR_OUT_TABLES = DIR_FIG / "marginal_comparison" / "tables"


# --------------------------------------------------------------------------------- #
# Load + unify the two row shapes to one (ESS, Rhat) per row.
# --------------------------------------------------------------------------------- #
def load_diag(n_chains: int = 2, *, density_agg: str = "worst",
...
def plot_ess_grid(n_chains: int = 2, df: Optional[pd.DataFrame] = None) -> ggplot:
    """ESS (log scale) of all three invariant series x 4 params, by k_true."""
    return _diag_grid(load_diag(n_chains, df=df), "ESS", n_chains)


def _diag_series(d: pd.DataFrame, metric: str, series_key: str, n_chains: int) -> ggplot:
    ...
    if metric == "Rhat":
        p = p + geom_hline(yintercept=RHAT_THRESH, linetype="dashed", color="#555555", size=0.7)
        ylab, title = "R-hat", f"{slabel} R-hat by True Components  (c{n_chains})"
    else:
        p = (p + scale_y_log10()
             + geom_hline(yintercept=ESS_MIN, linetype="dashed", color="#555555", size=0.7))
        ylab, title = "ESS  (log scale)", f"{slabel} ESS by True Components  (c{n_chains})"
    return (
...
def plot_density_rhat(n_chains: int = 2, df: Optional[pd.DataFrame] = None) -> ggplot:
    """R-hat of the marginal density (the primary functional, Eq. 5.5.19), by param and k_true."""
    return _diag_series(load_diag(n_chains, df=df), "Rhat", "density", n_chains)


def plot_density_ess(n_chains: int = 2, df: Optional[pd.DataFrame] = None) -> ggplot:
    """ESS of the marginal density (log scale), by param and k_true."""
    return _diag_series(load_diag(n_chains, df=df), "ESS", "density", n_chains)
...
def make_plots(n_chains: int = 2) -> None:
    d = load_diag(n_chains)
    print("wrote", save(_diag_grid(d, "Rhat", n_chains),
                        f"{DIR_OUT_PLOTS}/marginal_rhat_grid_c{n_chains}.png"))
    print("wrote", save(_diag_grid(d, "ESS", n_chains),
                        f"{DIR_OUT_PLOTS}/marginal_ess_grid_c{n_chains}.png"))
    print("wrote", save(_diag_series(d, "Rhat", "density", n_chains),
                        f"{DIR_OUT_PLOTS}/marginal_density_rhat_c{n_chains}.png"))
    print("wrote", save(_diag_series(d, "ESS", "density", n_chains),
                        f"{DIR_OUT_PLOTS}/marginal_density_ess_c{n_chains}.png"))


def main() -> None:
    write_tables()
    make_plots()
    print("marginal-diagnostics tables + figures -> analysis/out/k5_results/marginal_comparison/")
