...
Comparisons are ALL pairwise combinations of the four samplers, generated from
SAMPLER_ORDER (bayesm, bayesm_gibbs, nuts, hmc): the LATER sampler in the order
challenges the EARLIER one, so bayesm (the reference) is always a baseline.

Every distance metric is stored on TWO evaluation grids ('full' and 'chebyshev', keyed
by the `grid` column); all tables and plots are produced once per grid (filename
suffix _<grid>).
...
For every (comparison, metric, k_true) it reports:
  ...
  p_value   - two-sided sign test (binomial) that win_rate != 0.5
    ...
    .venv/Scripts/python.exe hpc_analysis/marginal_winrate.py
"""
...
# combinations of the four samplers; the later sampler in SAMPLER_ORDER challenges the
...
FIG_DIR = Path(__file__).resolve().parent / "out" / "marginal_comparison"
def _out_dir(grid):
    return FIG_DIR / GRID_FOLDER[grid] / "tables"
...
def win_rate_plot(comparison: str, tbl: pd.DataFrame, grid: str = "") -> ggplot:
    ...
    `grid` only annotates the title (the table is already per-grid).
    """
...
def main():
    for grid in GRIDS:
        ...
        # (comparison, metric, k_true, param).
        ...
        # Plots: one figure per comparison (win rate vs k_true, line per param, faceted by
        # metric). Guarded so a missing sampler skips its comparisons instead of aborting.
        for challenger, baseline in COMPARISONS:
        ...
        for comp in tbl["comparison"].unique():
            ...
            print(f"########## {comp} [{grid}]  (datasets where {chall} is closer, out of ~100 per k_true) ##########")
            for metric in MARGINAL_METRICS:
