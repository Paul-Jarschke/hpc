...
2. **Figures**: `.venv/Scripts/python.exe hpc_analysis/standard_model/make_plots.py`
   ...
   (delta, mu, sigma, beta, runtime, marginal comparison incl. per-grid win-rate
   figures + tables and marginal-series ESS/R-hat diagnostics, consolidated RMSE).
...
`marginal_winrate.py` and `marginal_diag.py` are also runnable standalone.
...
- `marginal_winrate.py` - paired head-to-head win rates on the marginal distances
  (nuts_vs_bayesm, hmc_vs_bayesm, hmc_vs_nuts), per grid.
