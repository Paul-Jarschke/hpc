"""
Convergence diagnostics (ESS and R-hat) of the marginal-comparison series, standard_model.

Source: data/out/standard_model/marginal_diagnostics.csv (src.summaries_standard ->
marginal_comparison.{density_series_diagnostics, moment_series_diagnostics}). With K = 1
there is no label switching, so these diagnostics of the reported functionals (the
marginal density, Eq. 5.5.19, and the mixture mean/variance, Eq. 5.5.2 with K = 1) sit
alongside the directly meaningful per-parameter diagnostics in convergence.csv.
...
The table mixes two row shapes; this module unifies them to one (ESS, Rhat) per row:
  ...
  * kind == 'density'  - the per-draw marginal density f(x) is a SERIES over draws evaluated
    ...
    ONE grid at a time and every output is produced per grid (full/trimmed subfolder).
...
Thresholds: R-hat <= 1.1 is the convergence gate used study-wide. ESS >= 400 is a
...
def _prep(d: pd.DataFrame) -> tuple[pd.DataFrame, list]:
    """Apply canonical ordered categoricals (sampler, series, param)."""
...
def _diag_grid(d: pd.DataFrame, metric: str, n_chains: int,
    ...
    """metric in {'Rhat','ESS'}: x = sampler, facet_grid(series x param).
    ...
    return (
        ...
        + labs(x="Sampler", y=ylab, color="Sampler", title=title)
        + theme_bw()
...
def plot_rhat_grid(n_chains: int = 2, df: Optional[pd.DataFrame] = None,
    ...
    """R-hat of all three invariant series (density / mean / variance) x 4 params, by sampler."""
...
def plot_ess_grid(n_chains: int = 2, df: Optional[pd.DataFrame] = None,
                  grid: str = "chebyshev") -> ggplot:
    """ESS (log scale) of all three invariant series x 4 params, by sampler."""
...
def _diag_series(d: pd.DataFrame, metric: str, series_key: str, n_chains: int,
                 grid: Optional[str] = None) -> ggplot:
    """Single-series view: facet_wrap by param (1x4), x = sampler.
    ...
    p = (
        ...
        + geom_boxplot(width=0.55, fill="#FFFFFF00", outlier_alpha=0)
    )
    if metric == "Rhat":
        p = p + geom_hline(yintercept=RHAT_THRESH, linetype="dashed", color="#555555", size=0.7)
        ylab, title = "R-hat", f"{slabel} R-hat by Sampler  (c{n_chains}{gtag})"
    else:
        p = (p + scale_y_log10()
             + geom_hline(yintercept=ESS_MIN, linetype="dashed", color="#555555", size=0.7))
        ylab, title = "ESS  (log scale)", f"{slabel} ESS by Sampler  (c{n_chains}{gtag})"
    return (
...
def plot_density_rhat(n_chains: int = 2, df: Optional[pd.DataFrame] = None,
                      grid: str = "chebyshev") -> ggplot:
    """R-hat of the marginal density (the primary functional, Eq. 5.5.19), by param."""
...
def plot_density_ess(n_chains: int = 2, df: Optional[pd.DataFrame] = None,
                     grid: str = "chebyshev") -> ggplot:
    """ESS of the marginal density (log scale), by param."""
...
def rhat_summary_table(n_chains: int = 2, d: Optional[pd.DataFrame] = None,
    ...
    """Per (series, param, sampler): median / q75 / max R-hat and the convergence
    ...
    g = d.groupby(["series", "param", "sampler"], observed=True)["Rhat"]
...
def ess_summary_table(n_chains: int = 2, d: Optional[pd.DataFrame] = None,
                      grid: str = "chebyshev") -> pd.DataFrame:
    """Per (series, param, sampler): ESS quartiles and the frac(ESS >= 400) pass-rate."""
    d = load_diag(n_chains, grid=grid) if d is None else d
    g = d.groupby(["series", "param", "sampler"], observed=True)["ESS"]
...
def _finish(out: pd.DataFrame, round_cols: list) -> pd.DataFrame:
    ...
    return out.sort_values(["series", "param", "sampler"]).reset_index(drop=True)
...
def write_tables(n_chains: int = 2) -> None:
    for grid in GRIDS:
        out_dir = _out_base() / GRID_FOLDER[grid] / "tables"
