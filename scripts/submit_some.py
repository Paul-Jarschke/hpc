...
# scripts/mark_finished_from_out.sh, so ~380 tasks remain per job). Submitted in LIST
# ORDER: NUTS first (it defines the total makespan), then HMC, then the fast Gibbs arms.
# 4 x 380 = 1520 tasks MAY exceed a per-user queued-task cap on scc medium; if a later
# sbatch is rejected (QOSMaxSubmitJobPerUserLimit), just re-run this script once the
